...
from gui_prompt import get_system_prompt
...
def run_react_loop(initial_intent: str, history: list, max_attempts: int, gui_client_url: str, show_img: bool, session_id: str, device_type: str = "pc"):
...
def run_agent_task(user_id: str, intent: str, history: list, max_attempts: int=5, gui_client_url: str=os.getenv("GUI_CLIENT_URL"), show_img: bool=False, device_type: str="pc"):
    ...
    if status == "running":
        ...
        result = run_react_loop(merged_intent, history, max_attempts, gui_client_url, show_img, session_id, device_type)
        return result
    ...
    try:
        ...
        # 直接使用 ReAct 循环处理总任务
        result = run_react_loop(intent, history, max_attempts, gui_client_url, show_img, session_id, device_type)
